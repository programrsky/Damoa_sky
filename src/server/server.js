...
app.post("/api/check", (req, res) => {
  ...
  app.post('/api/register', async (req, res) => {
    const { user_id, user_pw, user_name, user_num, user_date, user_email } = req.body;
  
    // Validate required fields
    if (!user_id || !user_pw || !user_name || !user_num || !user_date || !user_email) {
      return res.status(400).json({ error: 'Bad Request: Missing required fields' });
    }
  
    try {
      // Check if user_id or user_num already exists
      const checkUserSql = 'SELECT user_id, user_num FROM user_info WHERE user_id = ? OR user_num = ? OR user_email = ?';
      db.query(checkUserSql, [user_id, user_num, user_email], async (err, result) => {
        if (err) {
          console.error('Error fetching data from MySQL:', err);
        ...
        if (result.length > 0) {
          ...
          const insertUserSql = `
            INSERT INTO user_info (user_id, user_pw, user_name, user_num, user_date, user_email)
            VALUES (?, ?, ?, ?, ?, ?)
          `;
          db.query(insertUserSql, [user_id, user_pw, user_name, user_num, user_date, user_email], (err, result) => {
            if (err) {
