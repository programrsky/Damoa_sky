...
-   06.08 ~ 06.09
    ...
    -   메인 페이지 기대되는 개봉작 모달 창 추가 및 css 추가
    -   메인 페이지 년도, 월 ,일 선택 달력 css 변경
    -   리뷰 페이지 장르 및 삭제버튼 추가 (로그인 후 본인 게시물만 삭제 추가) -> 삭제 기능구현 완료
    -   커뮤니티 페이지 조회수 0.5씩 오르는 에러(새로고침 시 버그 발생) 수정 -> 1로 수정 완료
    -   리뷰 작성 후 추가 기능 구현 완료
    -   메인 기대되는 개봉작 우측 좌측 버튼 이동시 컴포넌트 크기 변경되는 점 수정
    -   하단바 왼쪽 글 변경
    -   커뮤니티 게시물 삭제버튼 추가
